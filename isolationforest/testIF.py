...
def loadmodel():
    ...
    tcp_file = glob.glob(f'isolation_forest_TCP*')
    mb_file = glob.glob(f'isolation_forest_MODBUS_*')
...
def detect(scaled_data, model, string):
    anom_scores = model.score_samples(scaled_data)
