...
def modelsave(model, string, scaler, encoders):
    ...
    filename = f"isolation_forest_{string}_{datestring}.pkl"
    ...
    scaler_filename = f"Scaler_{string}_{datestring}.pkl"
    pickle.dump(scaler, open(scaler_filename, 'wb'))

    if encoders != 'None':
        encoders_filename = f"Encoders_{string}_{datestring}.pkl"
...
def threshsave(tcp_threshold, mb_threshold):
    if os.path.exists('thresholdIF.py'):
        os.remove('thresholdIF.py')
    with open('thresholdIF.py', 'w') as f:
